...
const RestaurantProfile = ({restaurant, isSeller, onUpdate}:props) => {
    ...
    const toggelOpenStatus = async () => {
        try {
            const {data} = await axios.post(`${restaurantService}/api/restaurant/status`,
                {status : !isOpen},
                {
                    headers: {
   ...
   const logoutHandler =async () => {
    ...
    await axios.post(`${restaurantService}/api/restaurant/status`,
                {status : false},
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <div className="mx-auto max-w-xl rounded-xl bg-white overflow-hidden">
        {
           ...
           <div className="flex items-center justify-between">
            ...
            <div className="flex items-center justify-between pt-3 border-t">
                <span className={`text-sm font-medium ${isOpen?"text-green-600" :
                     "text-red-500"}`}>{isOpen?"OPEN":"CLOSED"}</span>
                     
                <div className="flex gap-3">
                    {
                        editMode && ( <button onClick={saveChanges} disabled={loading} 
                            className="flex items-center gap-1 rounded-lg bg-blue-500 px-3 py-1.5 
                            text-sm text-white hover:bg-blue-700"> <BiSave size={18} />Save</button> )
                    }

                    {
                        isSeller && ( <button onClick={toggelOpenStatus} 
                        className={`rounded-lg px-1.5 text-sm font-medium text-white ${isOpen 
                    ...
                    {
                        isSeller && ( <button onClick={logoutHandler} 
                        className={`rounded-lg px-1.5 text-sm font-medium text-white bg-red-500 hover:bg-red-700
                        }`}>
                             Logout </button> )
                    }
                </div>
