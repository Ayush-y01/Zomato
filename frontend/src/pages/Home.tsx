...
const Home = () => {
  ...
  const fetchRestaurants = async() => {
    if (!location?.latitude || !location?.longitude) {
      // alert("You Need To access Location!!!")
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 ">
      {
        restaurants.length >0 ? <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 
        md:grid-cols-4">
          {
            restaurants.map((res) => {
              ...
              return <RestaurantCards key={res._id} id={res._id} name={res.name} image={res.image ?? ""} 
              distance={`${distance}`} isOpen={res.isOpen} />
            })
          }

        </div>: <p className="text-center text-gray-500"> No Restaurant Found!!!!</p>
      }
    </div>
  )
}
